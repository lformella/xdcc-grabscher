			...
			else if (tChan != null)
			{
				...
				if (!isParsed)
				{
					...
					if (tMatch.Success)
					{
						...
						if (tBot.InfoSlotCurrent > 0 && tBot.State == BotState.Waiting)
						{
							tBot.State = BotState.Idle;
			...
			if (tBot != null)
			{
				...
				if (!isParsed)
				{
					...
					if (tMatch1.Success || tMatch2.Success || tMatch3.Success)
					{
						...
						isParsed = true;
						if (tBot.State == BotState.Idle)
						{
							tBot.State = BotState.Waiting;
						}
				...
				if (!isParsed)
				{
					...
					{
						isParsed = true;
						if (tBot.State == BotState.Waiting)
						{
							tBot.State = BotState.Idle;
						}
				...
				if (!isParsed)
				{
					...
					if (tMatch1.Success || tMatch2.Success || tMatch3.Success || tMatch4.Success || tMatch5.Success)
					{
						...
						else if(tBot.State == BotState.Waiting)
						{
				...
				if (!isParsed)
				{
					...
					if (tMatch1.Success || tMatch2.Success)
					{
						...
						{
							if (valueInt == 30 && tBot.State != BotState.Active)
							{
								tBot.State = BotState.Idle;
							}
