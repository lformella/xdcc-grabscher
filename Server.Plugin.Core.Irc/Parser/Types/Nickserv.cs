		...
		protected override bool ParseInternal(IrcConnection aConnection, string aMessage, IrcEventArgs aEvent)
		{
			if (aEvent.Data.Nick != null && aEvent.Data.Nick.ToLower() == "nickserv")
			{
				...
				if (Helper.Match(aMessage, ".*Your nick isn't registered.*").Success)
				{
					...
					{
						FireSendData(aConnection.Server, aEvent.Data.Nick + " register " + Settings.Instance.IrcPasswort + " " + Settings.Instance.IrcRegisterEmail);
					}
				}

				else if (Helper.Match(aMessage, ".*Nickname is .*in use.*").Success)
				{
					FireSendData(aConnection.Server, aEvent.Data.Nick + " ghost " + Settings.Instance.IrcNick + " " + Settings.Instance.IrcPasswort);
					FireSendData(aConnection.Server, aEvent.Data.Nick + " recover " + Settings.Instance.IrcNick + " " + Settings.Instance.IrcPasswort);
					FireSendData(aConnection.Server, "nick " + Settings.Instance.IrcNick);
				}

				else if (Helper.Match(aMessage, ".*Services Enforcer.*").Success)
				{
					FireSendData(aConnection.Server, aEvent.Data.Nick + " recover " + Settings.Instance.IrcNick + " " + Settings.Instance.IrcPasswort);
					FireSendData(aConnection.Server, aEvent.Data.Nick + " release " + Settings.Instance.IrcNick + " " + Settings.Instance.IrcPasswort);
					FireSendData(aConnection.Server, "nick " + Settings.Instance.IrcNick);
				}
				...
				else if (Helper.Match(aMessage, ".*(This nickname is registered and protected|This nick is being held for a registered user|msg NickServ IDENTIFY).*").Success)
				{
					if (Settings.Instance.IrcPasswort != "" && !_authenticatedServer.Contains(aConnection.Server))
					{
						...
						FireSendData(aConnection.Server, aEvent.Data.Nick + " identify " + Settings.Instance.IrcPasswort);
					}
					else
					{
				...
				else if (Helper.Match(aMessage, ".*Please type .*to complete registration.*").Success)
				{
					...
					if (tMatch.Success)
					{
						FireSendData(aConnection.Server, "/msg NickServ confirm " + tMatch.Groups["code"]);
