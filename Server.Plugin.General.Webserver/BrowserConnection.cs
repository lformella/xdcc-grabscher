	...
	public class BrowserConnection : APlugin
	{
				...
				if (str.StartsWith("/?"))
				{
					...
					switch (tMessage)
					{
						...
						case ClientRequest.SearchPacket:
							...
							response = Objects2Json(Packets(showOfflineBots, tDic["searchBy"], HttpUtility.UrlDecode(tDic["name"]), tDic["sidx"], tDic["sord"]),
							                        int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;

						case ClientRequest.SearchBot:
							Context.Response.ContentType = "text/json";
							response = Objects2Json(Bots(showOfflineBots, tDic["searchBy"], HttpUtility.UrlDecode(tDic["name"]), tDic["sidx"], tDic["sord"]),
							                        int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
						...
						case ClientRequest.Object:
							Context.Response.ContentType = "text/json";
							response = Object2Json(Servers.WithGuid(guid));
							break;

						case ClientRequest.Servers:
							Context.Response.ContentType = "text/json";
							response = Objects2Json(SortedObjects(Servers.All, tDic["sidx"], tDic["sord"]), int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
						...
						case ClientRequest.ChannelsFromServer:
							...
							response =
								Objects2Json(
								             SortedObjects(from server in Servers.All from channel in server.Channels where channel.ParentGuid == guid select channel,
								                           tDic["sidx"], tDic["sord"]), int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
							...
							response =
								Objects2Json(
								             SortedBots(
								                        ...
								                        where bot.ParentGuid == guid
								                        select bot, tDic["sidx"], tDic["sord"]), int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
						...
						case ClientRequest.PacketsFromBot:
							...
							response = Objects2Json(SortedPackets(from server in Servers.All
							                                      ...
							                                      where packet.ParentGuid == guid
							                                      select packet, tDic["sidx"], tDic["sord"]), int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
						...
						case ClientRequest.Files:
							Context.Response.ContentType = "text/json";
							response = Objects2Json(SortedFiles(Files.All, tDic["sidx"], tDic["sord"]), int.Parse(tDic["page"]), int.Parse(tDic["rows"]));
							break;
		...
		IEnumerable<Bot> Bots(bool aShowOfflineBots, string aSearchBy, string aSearchString, string aSortBy, string aSortMode)
		{
			...
			IEnumerable<Bot> tBots =
				(from s in Servers.All from c in s.Channels from b in c.Bots join p in tPacketList on b.Guid equals p.ParentGuid select b).Distinct();
		...
		IEnumerable<Packet> Packets(bool aShowOfflineBots, string aSearchBy, string aSearchString, string aSortBy, string aSortMode)
		{
			...
			{
				case "name":
					string[] searches = aSearchString.ToLower().Split(new[] {' '}, StringSplitOptions.RemoveEmptyEntries);
