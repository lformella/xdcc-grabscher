	...
	public class BotConnection : AIrcConnection
	{
		...
		protected override void ConnectionConnected()
		{
			...
			if (Part != null)
			{
				...
				Part.State = FilePart.FilePartState.Open;
				Part.Packet = Packet;
				...
				try
				{
					...
					Packet.Parent.State = Bot.BotState.Active;
		...
		protected override void ConnectionDisconnected(SocketErrorCode aValue)
		{
			...
			Packet.Parent.State = Bot.BotState.Idle;
			Packet.Parent.Commit();
			...
			if (Part != null)
			{
				...
				if (RemovePart)
				{
					Part.State = FilePart.FilePartState.Broken;
				...
				else
				{
					...
					{
						Part.State = FilePart.FilePartState.Ready;
